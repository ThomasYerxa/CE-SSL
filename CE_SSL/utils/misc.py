...
class UpdateTau(Callback):
    def __init__(self, MomentumCallback, base_tau, final_tau):
        super(UpdateTau, self).__init__()
        self.MomentumCallback = MomentumCallback
        self.base_tau = base_tau
        self.final_tau = final_tau
        self.max_steps = 625 * 100 # Hard coded for now, come back to this
        self.cur_step = 1
    
    def batch_end(self, state: State, logger: Logger):
        cur_tau = (
            self.final_tau
            - (self.final_tau - self.base_tau) * (math.cos(math.pi * self.cur_step / self.max_steps) + 1) / 2
        )

        self.MomentumCallback.tau = cur_tau

        self.cur_step += 1


class MomentumPairedUpdate(Callback):
    def __init__(self, tau=0.99) -> None:
        super(MomentumPairedUpdate, self).__init__()
        self.tau = tau
        self.first_time = True
    ...
    def batch_end(self, state: State, logger: Logger) -> None:
        ...
        online_g_1, momentum_g_1 = (
            state.model.module.module.g_1,
            state.model.module.module.mom_g_1,
        )

        online_g_2, momentum_g_2 = (
            state.model.module.module.g_2,
            state.model.module.module.mom_g_2,
        )
        ...
        with torch.no_grad():
            ...
            for op, mp in zip(online_g_1.parameters(), momentum_g_1.parameters()):
                mp.data = self.tau * mp.data + (1 - self.tau) * op.data

            for op, mp in zip(online_g_2.parameters(), momentum_g_2.parameters()):
                mp.data = self.tau * mp.data + (1 - self.tau) * op.data
...
class ShufflePairs(Callback):
    def __init__(self):
        super(ShufflePairs, self).__init__()

    def epoch_start(self, state: State, logger: Logger) -> None:
        state.train_dataloader.dataset.reset_splits()
        return None
